...
class TestBlockStore:
    def test_store_and_fetch_roundtrip(self, store):
        block = DataBlock.build(b"hello", elem_size=1, elem_align=1)
        digest = store.store(block)
        fetched = store.fetch(digest)
        assert isinstance(fetched, DataBlock)
        assert bytes(fetched.get_data()) == b"hello"
    ...
    def test_len_after_stores(self, store):
        block1 = DataBlock.build(b"one", elem_size=1, elem_align=1)
        block2 = DataBlock.build(b"two", elem_size=1, elem_align=1)
        store.store(block1)
        store.store(block2)
        assert len(store) == 2
