...
class TestFixedArrayTypeDecode:
    ...
    def test_decode_block_entry(self, store):
        ...
        block = DataBlock.build(data_bytes, elem_size=4, elem_align=4)
    ...
    def test_decode_link_wrong_limit(self, store):
        ...
        with pytest.raises(ValueError, match="limit 3, got 99"):
            fa.decode(table[0], store)
    ...
    def test_decode_inline_raises(self, store):
        ...
        fa = FixedArrayType(U32, 1)
        table = TableBlock.build([TableEntryRaw(TableEntryType.INLINE, 42)], b"")
        with pytest.raises(ValueError, match="Could not resolve entry to block"):
            fa.decode(table[0], store)
