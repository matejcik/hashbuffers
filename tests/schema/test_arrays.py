...
class Row(HashBuffer):
    cells: list[int] | None = Field(0, Array(U16))


class Grid(HashBuffer):
    rows: list[Row] | None = Field(0, Array(Row))
...
class TestArrayOfArrays:
    ...
    def test_roundtrip(self, store):
        class VarOfVar(HashBuffer):
            arrays: list[list[int]] | None = Field(0, Array(Array(U32)))
    ...
    def test_nested_bytes_arrays(self, store):
        ...
        class NestedBytes(HashBuffer):
            groups: list[list[bytes]] | None = Field(0, Array(Array(Bytes)))
